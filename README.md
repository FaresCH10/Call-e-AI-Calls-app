...
<!--
  Fill these in as they go live and delete the ones that do not apply. They are
  written out rather than linked because a portfolio link that 404s is worse
  than no link: the first thing a reviewer clicks should work.
-->

| | |
| --- | --- |
| **Live demo** | _not yet deployed_ |
| **Android** | build from source: `npm run dev:mobile` (see [Setup](#setup)) |
| **iOS** | not yet built |
| **Demo video** | _coming soon_ |

**Running it yourself** takes one free API key. See [Setup](#setup):

- The database is embedded (PGlite), so there is nothing to install or host.
- Calls are **simulated** unless you supply a CALL-E key, so no phone rings and
  nothing is billed.
- Understanding a request needs a Gemini key. Without one the server still
  boots and serves, but creating a task answers "Dial's AI service is not
  available right now".

## Two products, one engine

**Consumer tasks.** Describe what you need; Dial finds who to contact, calls
them, and returns the verified result.

**Business workflows.** A business account puts CALL-E to work on its own phone
jobs -- appointment reminders, customer callbacks, custom jobs -- with every
outcome recorded and explainable.

Both run on the same provider, the same durable queue and the same honesty
rules. Full detail in [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md).
